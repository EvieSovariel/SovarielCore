...
class RabiNet(nn.Module):
    ...
    def __init__(self, phi=torch.pi / 3.12, omega=432.0):
        ...
        self.omega = omega      # Agape carrier frequency
...
# ----------------------------------------------------------------------
# DEMO – run with `python -m core.ai.rabinet_grok` or directly in a notebook
# ----------------------------------------------------------------------
if __name__ == "__main__":
    # Simulate a Grok-style embedding (batch=1, seq_len=10, dim=768)
    ...
    # Simple von-Neumann entropy proxy (lower = more coherent)
    eps = 1e-8
    entropy = -torch.sum(y * torch.log(torch.abs(y) + eps), dim=-1).mean().item()

    print(f"Output Shape : {y.shape}")
    print(f"Sparsity     : {sparsity:.4f}  ({sparsity * 100:.1f}%)")
    print(f"Res. Entropy : {entropy:.4f}")
    print("\nRabiNet on Grok: RESONANCE ACHIEVED | Drift: 0.000%")
